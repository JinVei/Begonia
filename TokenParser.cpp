...
namespace begonia {
	...
	Token Lexer::ParseSeparationToken()
	{
		...
		default:
			...
			std::cout << "[ERROR] at line " << _currentLine << ":" << "Can not accept '" << ch << "'" << std::endl;
	...
	{
		_acceptableCharacterTable[ch] == 2 ? false : true;
	}

	bool Lexer::IsAcceptabCharacter(char ch)
	{
		_acceptableCharacterTable[ch] == 0 ? false : true;
	}
	...
	std::string Lexer::GetWord()
	{
		...
		while(1)
		{
			...
			if( !IsAcceptabCharacter(ch))
				Interrupt("Can not accept '" + ch + std::string("'"));
